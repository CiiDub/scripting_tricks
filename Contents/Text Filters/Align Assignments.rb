...
def print_line( new_line:, last_line: false )
	if @last_input_char != "\n" && last_line
...
# Groups adjacent assignments with the same leading whitespace together, spacing is made relative to these groups.
...
@output.each_with_index  do | line, index |
	if line.class == MatchData
		...
		last_line = true if index == ( @output.size - 1 )

		print_line( new_line: "#{indent}#{var}#{gap}#{op}#{op_pad}#{stuff}\n", last_line: last_line )
	else
		print_line( new_line: "#{line}" )
	end
end
